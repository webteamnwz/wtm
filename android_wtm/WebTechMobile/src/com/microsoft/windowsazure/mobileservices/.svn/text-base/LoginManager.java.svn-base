...
class LoginManager
{
	 ...
	 *            Callback to invoke when the authentication process finishes
	 */
	protected void showLoginUI(MobileServiceAuthenticationProvider provider, final String startUrl, final String endUrl, final Context context, LoginUIOperationCallback callback)
	{
	...
	private MobileServiceUser createUserFromJSON(JsonObject json) throws MobileServiceException
	{
		...
		else // If the JSON contains an error property show it, otherwise raise
